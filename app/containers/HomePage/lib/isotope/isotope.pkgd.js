...
(function(window, factory) {
  ...
  function getStyle(elem) {
    ...
    if (!style) {
      logError(
        `Style returned ${style}. Are you running this code in a hidden iframe on Firefox? ` +
...
(function(window, factory) {
  ...
  proto._transitionTo = function(x, y) {
    ...
    let compareX = parseInt(x, 10);
    let compareY = parseInt(y, 10);
    let didNotMove =
  ...
  proto.reveal = function() {
    ...
    let { options } = this.layout;

    const onTransitionEnd = {};
    let transitionEndProperty = this.getHideRevealTransitionEndProperty(
      'visibleStyle',
    );
...
(function(window, factory) {
  ...
  function Outlayer(element, options) {
    ...
    if (!queryElement) {
      if (console) {
        console.error(
          `Bad element for ${this.constructor.namespace}: ${queryElement ||
            element}`,
        );
      }
      return;
...
(function(window, factory) {
  ...
  proto.updateSortData = function() {
    ...
    let sorters = this.layout._sorters;
    for (let key in getSortData) {
      let sorter = sorters[key];
      this.sortData[key] = sorter(this.element, this);
    }
  };

  let _destroy = proto.destroy;
  proto.destroy = function() {
    // call super
...
(function(window, factory) {
  ...
  let Vertical = LayoutMode.create('vertical', {
    horizontalAlignment: 0,
  });

  let proto = Vertical.prototype;
...
(function(window, factory) {
  ...
  proto._bindArrangeComplete = function() {
    ...
    let isLayoutComplete;
    var isHideComplete;
    let isRevealComplete;
    const _this = this;
  ...
  proto._sort = function() {
    ...
    // sort magic
    const itemSorter = getItemSorter(
      this.sortHistory,
